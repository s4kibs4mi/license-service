...
import ninja.sakib.licenseservice.exceptions.JwtTokenInvalidException;
...
public class JwtAuthFilterService extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        Claims parsedClaims = securityService.parseToken(token);

        if (securityService.isTokenExpired(token)) {
            throw new JwtTokenInvalidException("jwt token expired");
        }

        String userId = parsedClaims.getSubject();
        UserDto userDto = userService.findUserById(userId);
        UserDetailsImpl user = new UserDetailsImpl(userDto.getId(), "");
