...
@Aspect
@Component
public class SvcLoggingAspect {

    @Before("execution(* ninja.sakib.licenseservice.services.*.*(..))")
    public void logBefore(JoinPoint joinPoint) {
        log.info("Entering method: {} with arguments: {}", joinPoint.getSignature(), joinPoint.getArgs());
    }

    @AfterReturning(value = "execution(* ninja.sakib.licenseservice.services.*.*(..))", returning = "result")
